...
//   cudaEventElapsedTime(&ms1, total_start, total_stop);
  ...
  if(P != 0)
  {
	...
	const __half* cov3D_precomp_ptr = nullptr;
	if (cov3D_precomp.size(0) > 0) {
		cov3D_precomp_ptr = reinterpret_cast<const __half*>(cov3D_precomp.contiguous().data_ptr<at::Half>());
	}


		const __half* scales_ptr = nullptr;
		if (sh_scales.size(0) > 0) {
			scales_ptr = reinterpret_cast<const __half*>(sh_scales.contiguous().data_ptr<at::Half>());
		}
