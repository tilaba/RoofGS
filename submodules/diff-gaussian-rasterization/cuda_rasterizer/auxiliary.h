...
#define TILE_HEIGHT 2 // 从 16 改为

// Spherical harmonics coefficients
__constant__ const float SH_C0 = 0.28209479177387814f;
__constant__ const float SH_C1 = 0.4886025119029199f;
__constant__ const float SH_C2[] = {
	1.0925484305920792f,
	-1.0925484305920792f,
...
__constant__ const float SH_C3[] = {
	-0.5900435899266435f,
	2.890611442640554f,
	-0.4570457994644658f,
	0.3731763325901154f,
...
__forceinline__ __device__ bool in_frustum_opt(int idx,
	float3 p_orig,
	const float* viewmatrix,
	const float* projmatrix,
	bool prefiltered,
    ...
    for (int u = rect_min.x; u < rect_max.x; ++u)
    {
        ...
        if (gaussian_keys_unsorted != nullptr) {
          ...
          for (int v = min_tile_v; v < max_tile_v; v++)
          {
            ...
            uint32_t tile_id = isY ?  (u * grid.x + v) : (v * grid.x + u);
            float z_near = 0.0;
            float z_far = max_depth;
            // float z_far = 35.0;
            float normalized_z = __saturatef((depth - z_near) / (z_far - z_near));
            uint32_t qdepth = (uint32_t)(normalized_z * QUANT_DEPTH_MASK);
            // uint64_t key = ((uint64_t)tile_id << QUANT_DEPTH_BITS) | qdepth;
            uint32_t key = (tile_id << QUANT_DEPTH_BITS) | qdepth;
...
__device__ inline uint32_t duplicateToTilesTouched(
    ...
    uint32_t idx, uint32_t off, float depth,
    uint32_t* gaussian_keys_unsorted,
    uint32_t* gaussian_values_unsorted, const float max_depth = 1)
{
    ...
    float x_term = __fsqrt_rn(-(con_o.y * con_o.y * t) / (disc * con_o.x));
    x_term = (con_o.y < 0) ? x_term : -x_term;
    float y_term = __fsqrt_rn(-(con_o.y * con_o.y * t) / (disc * con_o.z));
